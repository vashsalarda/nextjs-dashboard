...
export async function fetchCustomersByQuery(query: string, page: number, pageSize: number) {
  let customerPage = {} as CustomerPage;
  try {
    customerPage = await customerService.getCustomersByQuery(query, page, pageSize);
  } catch (error) {
    console.error('Database Error:', error);
    // throw new Error('Failed to fetch customers');
  }
...
export async function fetchCustomersInvoices(query: string) {
  let customers = [] as CustomersTableType[]
  try {
    customers = await customerService.getInvoicesCustomers(query);
  } catch (error) {
    console.error('Database Error:', error);
    // throw new Error('Failed to fetch customers');
  }
